...
signed main(signed argc, char *argv[]) {
	...
	dump(A);
	dump(B);
	set<int>a; rep(i, 0, N) { a.insert(A[i]); }
	set<int>b; rep(i, 0, M) { b.insert(B[i]); }
	if (a.size() != A.size() or b.size() != B.size()) {
		cout << 0 << endl;
		return 0;
	}
	mint ans = 1;
	rrep(i, 1, N*M + 1) {
		...
		if (x and y)continue;
			...
			ans *= M - (lower_bound(all(B), i) - B.begin());
		}
		else if (y) {
			ans *= N - (lower_bound(all(A), i) - A.begin());
		}
		else {
			int g= N - (lower_bound(all(A), i) - A.begin());
			int h= M - (lower_bound(all(B), i) - B.begin());
			ans *= g * h - (N*M - i);
		}
	}
	cout << ans << endl;
