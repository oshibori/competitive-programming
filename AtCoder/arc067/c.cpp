...
mint modpow(mint base, long long exponent) {
  mint res = 1;
  while (exponent > 0) {
    if (exponent & 1)
      res = res * base;
...
signed main(signed argc, char *argv[]) {
  ...
  int N;
  cin >> N;
  nCr_compute_factinv(N + 100);
  auto f = [&](int N, int i, int k) {
    mint ret = fact[N];
    return ret / modpow(fact[k], i) / fact[N - i * k] / fact[i];
  };
  int a, b, c, d;
  cin >> a >> b >> c >> d;
  mint dp[1010][1010];
  ...
  rep(i, 1, N + 1) rep(j, 0, N + 1) {
    ...
    dp[i][j] = dp[i - 1][j];
    if (a <= i and i <= b) {
      for (int k = c; k * i <= j and k <= d; k++) {
        dp[i][j] += dp[i - 1][j - k * i] * f(j, k, i);
      }
    }
  }
  cout << dp[N][N] << endl;
