...
int ccw(Point p0, Point p1, Point p2) {
  ...
  if (cross(a, b) < -EPS)
    return CLOCKWISE;
  if (dot(a, b) < -EPS)
    return ONLINE_BACK; // p2 p0 p1
  if (a.norm() < b.norm())
    return ONLINE_FRONT; // p0 p1 p2
  return ON_SEGMENT;
}
...
double get_distance(Segment s1, Segment s2) {
  if (intersect(s1, s2))
    return 0.0;
  return min(min(get_distance(s1, s2.p1), get_distance(s1, s2.p2)),
...
Polygon convex_hull(Polygon P) {
  ...
  for (Point &p : P) {
    while (up.size() > 1 &&
           ccw(up[up.size() - 2], up[up.size() - 1], p) != CLOCKWISE)
      up.pop_back();
    up.emplace_back(p);
  }
  Polygon down;
  for (Point &p : P) {
    while (down.size() > 1 &&
           ccw(down[down.size() - 2], down[down.size() - 1], p) !=
               COUNTER_CLOCKWISE)
      down.pop_back();
    down.emplace_back(p);
  }
...
Polygon convex_hull_with_points_online(Polygon P) {
  ...
  for (Point &p : P) {
    int _ccw;
    while (up.size() > 1 &&
           (_ccw = ccw(up[up.size() - 2], up[up.size() - 1], p)) != CLOCKWISE &&
           _ccw != ONLINE_FRONT)
      up.pop_back();
    up.emplace_back(p);
  }
  ...
  for (Point &p : P) {
    int _ccw;
    while (down.size() > 1 &&
           (_ccw = ccw(down[down.size() - 2], down[down.size() - 1], p)) !=
               COUNTER_CLOCKWISE &&
           _ccw != ONLINE_FRONT)
      down.pop_back();
    down.emplace_back(p);
...
Polygon rotate(const Polygon &P, double rad) {
  Polygon ret;
  for (auto &p : P)
    ret.emplace_back(p.x * cos(rad) - p.y * sin(rad),
...
signed main(signed argc, char *argv[]) {
  ...
  dump(d);
  Polygon polygon;
  polygon.eb(Point(N, d));
  polygon.eb(Point(d, N));
  polygon.eb(Point(0, d));
  polygon.eb(Point(d, 0));
  dump(polygon);
  dump(contains(polygon, Point(d, d)));

  int ans = 0;
  rep(i, 0, N) {
    rep(j, 0, N) {
      bool f = true;
      f &= !!contains(polygon, Point(i, j));
      dump(f);
      f &= !!contains(polygon, Point(i + 1, j));
      dump(f);
      f &= !!contains(polygon, Point(i, j + 1));
      dump(f);
      f &= !!contains(polygon, Point(i + 1, j + 1));
      dump(f);
      ans += f;
    }
  }
