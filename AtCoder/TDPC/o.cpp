...
signed main(signed argc, char *argv[]) {
  ...
  len[0] = v[0];
  rep(i, 1, v.size()) { len[i] = len[i - 1] + v[i]; }

  vector<vector<mint>> dp(v.size(), vector<mint>(len[n - 1] + 1, 0));
  dp[0][len[0] - 1] = 1;
  rep(i, 0, n - 1) {
    rep(j, 0, len[i]) {
      ...
      // v[i+1]-1 C k-1 1<=k
      rep(k, 1, v[i + 1] + 1) {
        mint a = nCr(v[i + 1] - 1, k - 1);
        rep(l, 0, min(j, k) + 1) {
          mint b = nCr(j, l);
          mint c = nCr(len[i] + 1 - j, k - l);
          dp[i + 1][j - l + v[i + 1] - k] += dp[i][j] * a * b * c;
        }
      }
    }
  }
