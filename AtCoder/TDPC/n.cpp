...
const int NN = 1010;
mint dp[1010][1010];
int _vn[1010][1010], checkdp[1010][1010];
int vn(int p, int a) {
  int &ret = _vn[p][a];
  if (ret != -1)
    return ret;
...
signed main(signed argc, char *argv[]) {
  ...
  for (auto p : edge) {
    int a, b;
    tie(a, b) = p;
    ans += fact[vn(a, b) + vn(b, a)] * dfs(a, b) * dfs(b, a) / fact[vn(a, b)] /
           fact[vn(b, a)];
  }
  cout << ans << endl;
